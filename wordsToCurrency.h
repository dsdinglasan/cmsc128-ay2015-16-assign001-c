...
void wordsToCurrency(){
	...
	char *array_currency[3] = {"JPY", "PHP", "USD"};
	long num=0;
	...
	for(i=0;i<size;i++){//while there is still word
		...
		for(a=0;a<3;a++){//comparing the current word to the word of currency PHP, JPY, USD
			if(strcmp(each_word, array_currency[a])==0){
				strcpy(currency, each_word);//copies the currency given by the user to currency
				break;
			}
		}
		//printf("%s: %ld\n", each_word, num);
